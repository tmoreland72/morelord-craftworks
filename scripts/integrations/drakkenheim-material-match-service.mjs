import { log, warn, error } from "../core/logger.mjs";

const Logger = Object.freeze({
  log, warn, error, debug: (...args) => console.debug("Morelord Craftworks |", ...args)
});
 ...
 */
export class DrakkenheimMaterialMatchService {
    static #materialRegistry = null;
    static #ingredientIndex = null;

    static configure(materialRegistry) {
        this.#materialRegistry = materialRegistry;
        this.#ingredientIndex = null;
    }

    static async getIngredientIndex({ force = false } = {}) {
        if (this.#ingredientIndex && !force) return this.#ingredientIndex;

        const registry = this.#materialRegistry;
        if (!registry) {
            Logger.warn("Drakkenheim material matcher has no MaterialRegistry.");
            return [];
        }
        ...
        this.#ingredientIndex = registry.all()
            .filter(entry => entry.packId === "monsters-of-drakkenheim")
            .map(entry => ({
                _id: entry.materialId,
                materialId: entry.materialId,
                name: String(entry.name ?? "").replace(/\s*\((?:Common|Uncommon|Rare|Very Rare|Legendary|Artifact)\)\s*$/i, ""),
                img: entry.img,
                type: "loot",
                uuid: entry.uuid,
                system: { rarity: rarityKeys[String(entry.rarity ?? "").toLowerCase()] ?? entry.rarity ?? null }
            }));

        Logger.log(`Indexed ${this.#ingredientIndex.length} internal Drakkenheim harvesting materials.`);
            ...
            index
                .map((ingredient) => {
                    ...
                    return {
                        ...
                        materialId:
                            ingredient.materialId ?? ingredient._id,
